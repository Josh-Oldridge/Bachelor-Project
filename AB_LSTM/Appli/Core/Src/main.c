...
/* Private typedef -----------------------------------------------------------*/
/* USER CODE BEGIN PTD */
/* ---- FEED MODE SWITCH (build-time) ---- */
typedef enum {
	FEED_LIVE = 0,           // TIM1 encoder → real windows
	FEED_SYNTH_NORMAL,       // synthetic "normal"
	FEED_SYNTH_BOUNCE,       // synthetic "bounce"
	FEED_SYNTH_MISSING,      // synthetic "missing_step"
	FEED_SYNTH_Z             // synthetic "z_index"
} enc_feed_mode_t;

/* Default if not overridden in Preprocessor Symbols */
#ifndef FEED_MODE
#define FEED_MODE FEED_LIVE
#endif
/* USER CODE END PTD */
...
extern volatile float last_y[];       // final
extern volatile float last_y_raw[];   // raw
extern volatile uint32_t enc_rule_flags; // live, latest window computed
extern volatile uint32_t win_rule_flags;
extern volatile uint32_t cc1_overruns, cc2_overruns;

static uint32_t last_inf_seen = 0;
static uint32_t last_inf_print = 0;
const uint32_t INF_PRINT_EVERY = 1;
...
static uint32_t drops_prev = 0;
static uint32_t cc1_prev = 0;
static uint32_t cc2_prev = 0;
/* USER CODE END PV */
...
#if (FEED_MODE == FEED_LIVE)
	HAL_TIM_Encoder_Start(&htim1, TIM_CHANNEL_ALL);
	HAL_TIMEx_EnableEncoderIndex(&htim1);
	__HAL_TIM_ENABLE_IT(&htim1, TIM_IT_CC1 | TIM_IT_CC2 | TIM_IT_IDX);
	htim1.Instance->CCER |= TIM_CCER_CC1E | TIM_CCER_CC2E; // Force enable captures on CH1/CH2
	htim1.Instance->ARR = 0xFFFF;
	printf("ENC MODE: LIVE (TIM1 enabled)\r\n");
#else
  /* Make sure HW is fully quiet and FIFOs are empty in synthetic modes */
  EncSampler_StopHardware();
  EncSampler_FlushWindows();
  printf("ENC MODE: SYNTHETIC (mode=%d)\r\n", (int)FEED_MODE);
#endif
	...
	while (1) {
		...
		if (inf_total != last_inf_seen) {
			last_inf_seen = inf_total;

			if ((inf_total - last_inf_print) >= INF_PRINT_EVERY) {
				last_inf_print = inf_total;
				...
				printf(
						"RAW=%s [%.3f, %.3f, %.3f, %.3f]  flags(win)=0x%08lX flags(live)=0x%08lX\r\n",
						MODEL_CLASSES[best_raw], (double) last_y_raw[0],
						(double) last_y_raw[1], (double) last_y_raw[2],
						(double) last_y_raw[3], (unsigned long) win_rule_flags,
		...
		if ((int32_t) (HAL_GetTick() - rate_t0_ms) >= 0) {
			...
			printf(
					"RATES: TIM7=%lu Hz  windows=%lu/s  inferences=%lu/s  fifo_ready=%lu\r\n",
...
#if (FEED_MODE == FEED_LIVE)
			uint32_t z_pin_state = HAL_GPIO_ReadPin(GPIOE, GPIO_PIN_7);
			printf("DROPS=%lu(+%lu) Z_PIN=%lu\r\n", (unsigned long) event_drops,
					(unsigned long) (event_drops - drops_prev),
					(unsigned long) z_pin_state);

			printf("OVERRUNS: CC1=%lu(+%lu)  CC2=%lu(+%lu)\r\n",
					(unsigned long) cc1_overruns,
					(unsigned long) (cc1_overruns - cc1_prev),
					(unsigned long) cc2_overruns,
					(unsigned long) (cc2_overruns - cc2_prev));

			drops_prev = event_drops;
			cc1_prev = cc1_overruns;
			cc2_prev = cc2_overruns;
#endif
			t7_prev = t7_now;
			win_prev = win_now;
			inf_prev = inf_now;
		...
		if (win_ready == 0)
			__WFI();
#else
  /* In synthetic modes, don’t sleep waiting for encoder windows. */
  HAL_Delay(1);
#endif
	}
	/* USER CODE END 3 */
