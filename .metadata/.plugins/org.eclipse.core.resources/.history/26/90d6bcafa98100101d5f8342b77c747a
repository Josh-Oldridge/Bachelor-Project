...
/* --- Fast pin reads from main.h symbols (kept for optional debugging) --- */
...
/* Speed_Change_Rate (interval-based) */
static uint32_t samps_since_ab_change = 0;  /* samples since last AB state change */
static float    ema_interval          = 0.0f;
static float    scr_hold              = 0.0f;
...
void EncSampler_Init(void) {
  /* nothing to init; we derive clean A/B from TIM1 phase */
}
...
void EncSampler_TIM7_Callback(void)
{
  ...
  /* Optional: store RAW pins for debugging; features won't use them */
  uint8_t A = read_A_hw();
  uint8_t B = read_B_hw();
...
MODEL_FEATURES order (must match model_meta.h):
  ...
  4 Speed_Change_Rate           <-- |interval/EMA - 1|
  5 Channel_A
  6 Channel_B
  7 Channel_Z
  8 Edge_RISING
  9 Steps_Since_Z               <-- +1 per tick since last Z
*/
void EncSampler_Process(void)
{
  while (rb_r != rb_w) {
    ...
    /* Steps_Since_Z must match training: +1 per sample, NOT +ad */
    if (z_pulse) { steps_since_z = 0; }
    else         { steps_since_z += 1; }

    /* -------- Synthesize clean A/B from TIM1 phase (cnt & 3) --------
       This uses the same filtered edges that update TIM1->CNT, so A/B,
       State_Change, Direction, etc. are consistent with 'd'.           */
    const uint8_t idx_curr = (uint8_t)(s.cnt & 3);     /* 0..3 phase */
    ...
    /* Quadrature sanity: with phase-derived A/B this should not trigger,
       but keep it as a guard. Only when exactly one channel changed and |d|<=1. */
    ...
    /* -------- Speed_Change_Rate = |interval/EMA(interval) - 1| -------- */
    samps_since_ab_change += 1;
    if (state_change) {
      uint32_t interval = samps_since_ab_change;
      if (interval == 0) interval = 1;
      if (ema_interval <= 0.0f) ema_interval = (float)interval;
      float ratio = ((float)interval) / ema_interval;     /* can exceed 1.0 */
      float scr   = ratio - 1.0f; if (scr < 0.f) scr = -scr;
      scr_hold    = scr;                                   /* hold until next change */
      /* slow EMA to mimic ‘Interval_Ratio’ baseline adaptation */
      ema_interval = 0.9f * ema_interval + 0.1f * (float)interval;
      samps_since_ab_change = 0;
    }
    const float scr_out = scr_hold;
...
#define PACK(v) do { f[k] = scale_feat(k, (v)); k++; } while(0)
    ...
    PACK(scr_out);                           // 4 Speed_Change_Rate
